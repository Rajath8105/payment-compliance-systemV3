...
import { AlertCircle, CheckCircle, XCircle, FileText, TrendingUp, Clock, DollarSign, AlertTriangle, Brain, Shield, Upload, BookOpen, Trash2 } from 'lucide-react';
...
const PaymentComplianceSystem = () => {
  ...
  const handleRulebookUpload = async (e, scheme) => {
    ...
    try {
      ...
      alert(`✅ Rulebook uploaded successfully!\n\nScheme: ${result.scheme}\nFile: ${result.filename}\nPages: ${result.pages_estimated}\n\nAI is now ready to validate ${scheme} payments using this rulebook.`);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-purple-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">PDF Rulebooks</p>
              <p className="text-3xl font-bold text-gray-900">{Object.keys(rulebooks).length}</p>
          ...
          {['SEPA', 'SWIFT_MT103', 'CHAPS', 'SIX'].map(scheme => (
            <div key={scheme} className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-blue-400 transition-colors">
              ...
              <label className={`w-full inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg cursor-pointer font-semibold ${
                ...
                />
                <Upload size={16} />
                {isUploadingRulebook ? 'Uploading...' : (rulebooks[scheme] ? 'Replace PDF' : 'Upload PDF')}
              </label>
            </div>
          ))}
        </div>
